...
namespace Shampan.Repository.SqlServer.ModulePermissions
{
	public class ModulePermissionRepository : Repository, IModulePermissionRepository
	{       
        public ModulePermissionRepository(SqlConnection context, SqlTransaction transaction)
		{
 ...
 select 
 ModulPermission.Id
,ModulPermission.UserId
,ModulPermission.Modul
,ModulPermission.ModulId
,ModulPermission.IsActive
,Au.UserName


,ModulPermission.CreatedBy
,ModulPermission.CreatedOn
,ModulPermission.CreatedFrom
,ModulPermission.LastUpdateBy
,ModulPermission.LastUpdateOn
,ModulPermission.LastUpdateFrom

                 from ModulPermission 

                 LEFT JOIN GDICAuditAuthDB.dbo.AspNetUsers Au ON Au.Id = ModulPermission.UserId 
		...
		public int GetCount(string tableName, string fieldName, string[] conditionalFields, string[] conditionalValue)
		{
            ...
            {
                sqlText = @"
                 select count(ModulPermission.Id)FilteredCount
                from ModulPermission  where 1=1 ";
        ...
        public List<ModulePermission> GetModulListData(IndexModel index, string[] conditionalFields, string[] conditionalValue, PeramModel vm = null)
        {
            string sqlText = "";
		...
		public ModulePermission Insert(ModulePermission model)
		{
			try
			{
				...
				var command = CreateCommand(@" INSERT INTO ModulPermission(
...
,@CreatedFrom
				...
				command.Parameters.Add("@Modul", SqlDbType.NVarChar).Value = model.Modul;
				command.Parameters.Add("@ModulId", SqlDbType.NVarChar).Value = model.ModuleName;
		...
		public ModulePermission Update(ModulePermission model)
		{
			try
			{
				...
				string query = @"  update ModulPermission set
...
where  Id= @Id ";
                ...
                command.Parameters.Add("@UserId", SqlDbType.NChar).Value = model.UserId;
                command.Parameters.Add("@Modul", SqlDbType.NChar).Value = model.Modul;
                command.Parameters.Add("@ModulId", SqlDbType.NChar).Value = model.ModuleName;
                command.Parameters.Add("@IsActive", SqlDbType.Bit).Value = model.IsActive;
