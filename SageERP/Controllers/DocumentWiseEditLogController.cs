...
namespace SSLAudit.Controllers
{
    ...
    [Authorize]
    public class DocumentWiseEditLogController : Controller
    {

        private readonly ApplicationDbContext _applicationDb;      
        private readonly IDocumentWiseEditLogService DocumentWiseEditLogService;



        public DocumentWiseEditLogController(ApplicationDbContext applicationDb, IDocumentWiseEditLogService documentWiseEditLogService)
        {

            _applicationDb = applicationDb;
			DocumentWiseEditLogService = documentWiseEditLogService;
        ...
        public ActionResult Create()
        {
            ...
            ModelState.Clear();
			DocumentWiseEditLog vm = new DocumentWiseEditLog();
            vm.Operation = "add";
        ...
        public ActionResult CreateEdit(DocumentWiseEditLog master)
        {
            ResultModel<DocumentWiseEditLog> result = new ResultModel<DocumentWiseEditLog>();
            try
            {

                if (master.Operation == "update")
                {
                    foreach (var item in master.DocumentWiseEditLogDetails)
                    {
                        item.Id = master.Id;
                        ...
                        result = DocumentWiseEditLogService.Update(item);
                    }
                    return Ok(result);
                }
                else
                {
                    ...
                    foreach (var item in master.DocumentWiseEditLogDetails)
                    {
                        ...
                        result = DocumentWiseEditLogService.Insert(item);
                    }
        ...
        public ActionResult Edit(int id)
        {
            try
            {
                ResultModel<List<DocumentWiseEditLog>> result =
					DocumentWiseEditLogService.GetAll(new[] { "Id" }, new[] { id.ToString() });

				DocumentWiseEditLog pre = result.Data.FirstOrDefault();
        ...
        public IActionResult _index()
        {
            try
            {
                ...
                ResultModel<List<DocumentWiseEditLog>> indexData =
					DocumentWiseEditLogService.GetIndexData(index, conditionalFields, conditionalValue);



                ResultModel<int> indexDataCount =
				DocumentWiseEditLogService.GetIndexDataCount(index, conditionalFields, conditionalValue);


                int result = DocumentWiseEditLogService.GetCount(TableName.DocumentWiseEditLogDetails, "Id", new[] { "DocumentWiseEditLog.createdBy", }, new[] { userName });
            ...
            {
                e.LogAsync(ControllerContext.HttpContext);
                return Ok(new { Data = new List<DocumentWiseEditLog>(), draw = "", recordsTotal = 0, recordsFiltered = 0 });
            }
